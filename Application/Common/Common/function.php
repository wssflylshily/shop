...
/* 根据name获取品牌下拉菜单 */
function parse_brand($name,$checkid=0)
{
	$list	=	M('Brand')->select();
	...
	return	$tree->getSelectList($name,$checkid,0,'请选择品牌');
	//$this->assign('brand_id_option',$brand_id_option);
}
...
function qrcode($user_id=0,$reurl)
{
		$path='./Uploads/qrcode/';
		$QR=$path.'qrcode_'.$user_id.'.png';
		if(!file_exists($QR))
		{
			vendor("phpqrcode.phpqrcode");
			$text=C('WEBSITE')."/Reg?user_id=".$user_id;
			//纠错级别， 纠错级别越高，生成图片会越大
			//L水平    7%的字码可被修正
			//M水平    15%的字码可被修正
			//Q水平    25%的字码可被修正
			...
			\QRcode::png($reurl, $QR,$level, $size);
		}
		return $QR;
