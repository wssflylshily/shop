...
class ProductModel extends Model{
    ...
    public function detail($id){
		...
		//var_dump($logic);exit;
        $detail = $logic->detail($id); //获取指定ID的数
    ...
    public function update($data = null){
        ...
        $this->__set('category_id',intval(I('category_id')));
		$this->__set('is_home',intval(I('is_home')));
		$this->__set('is_cat',intval(I('is_cat')));
		$this->__set('is_integral',intval(I('is_integral')));
		...
		$row=M('Model')->field('name')->where('id='.$data['model_id'])->find();
		$model	=	D($row['name']);
		$data = $model->create();
		if($data === false){
        ...
        hook('productSaveComplete', array('model_id'=>$data['model_id']));

        //行为记录
        if($id){
        	action_log('add_product', 'product', $id, UID);
        }
    ...
    private function logic($model){
		//echo get_document_model($model, 'name');exit;
