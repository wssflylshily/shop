...
class ProductController extends AdminController {
    ...
    public function index($cate_id = null){
        ...
        //获取模型信息
        $model = M('Model')->getByName('product_model');
    ...
    protected function indexOfArticle($cate_id){
        ...
        $Product = M('Product');
        $map['category_id'] =   $cate_id;
		$catelist=M('Category')->where('pid='.$cate_id)->select();
		if($catelist)
		{
			$categoryids[]=$cate_id;
			...
			$map['category_id']=array('in',implode(',',$categoryids));
		}
        $map['pid']         =   I('pid',0);
        if($map['pid']){ // 子文档列表忽略分类
            unset($map['category_id']);
    ...
    public function add(){
		...
		//获取仓库列表
		$depotlist	=	M('Depot')->select();
		$this->assign('depotlist',$depotlist);

        /* 获取要编辑的扩展模型模板 */
        $model      =   get_document_model($model_id);
		//var_dump(parse_config_attr($model['field_group']));exit;
        //处理结果
    ...
    public function edit(){
        ...
        $cate =M('category')->where('id ='.$cate_id)->getField('pid');
        if ($cate!='0') {
            # code...
             $cateArr =M('category')->where('pid='.$cate)->select();
        }else{
            $cateArr='';
        }
		...
		//获取属性及值
		$titles	=	explode(',',$data['attrs_titles']);
		$values	=	explode(',',$data['attrs_values']);
		$i=1;
		...
		$attrlist=M('ProductAttr')->where('product_id='.$data['id'])->select();
// 		print_r($attrlist);die;
		if ($attrlist){
		//echo M('ProductAttr')->getlastsql();exit;
			foreach($attrlist as $item)
			{
				$pricearr[$item['attr_value']]=array($item['price']);
				$oldpricearr[$item['attr_value']]=array($item['oldprice']);
				$stockarr[$item['attr_value']]=array($item['stock']);
			}
			//var_dump($pricearr);exit;
			...
			foreach($list as $key=>$item)
			{
				...
				$attr_html.='<div style="height:20px;">
							<span id="price_'.$key.'">
							<label class="item-label2">'.$html.'</label>&nbsp;&nbsp;现价格：<input type="text" name="price['.$key.']['.$xb.']" class="text input" value="'.$pricearr[$value][0].'">&nbsp;&nbsp;
							原价格：<input type="text" name="oldprice['.$key.']['.$xb.']" class="text input" value="'.$oldpricearr[$value][0].'">&nbsp;&nbsp;
							库存数量：<input type="text" name="stock['.$key.']['.$xb.']" class="text input" value="'.$stockarr[$value][0].'">
							</span>
							</div><br>';
			}
			//echo $attr_html;exit;
			$this->assign('attr_html',$attr_html);
			if ($data['attrs_titles']){
				$titles=explode(',',$data['attrs_titles']);
				$attr_str='';
				foreach($titles as $key=>$item)
				{
					$attr_str.='<a class="btn" id="item_'.$key.'" onclick="delitem(\''.$titles[$key].'\','.$key.')">'.$titles[$key].'</a>&nbsp;&nbsp;';
				}
			
			}
// 			print_r($attr_str);die;
        ...
        if(!$res){
			...
			if($attrs_titles&&$attrs_values){
				...
				foreach($list as $key=>$item)
				{
					...
					$data['oldprice']=$_POST['oldprice'][$key][$xb];
					$data['stock']=$_POST['stock'][$key][$xb];
					//var_dump($data);exit;
	...
	public function upstatus()
	{
		$id=intval(I('id'));
		$status=I('status');
		M('Product')->where('id='.$id)->save(array('status'=>$status));
		if($status==0){
			M('cart')->where('product_id='.$id.' and status=1')->save(array('status'=>0));	
		}
		$this->success('操作成功');
	}
	...
	{
		$attr_titles=I('post.attr_titles');
		...
		foreach($list as $key=>$item)
		{
			...
			$attr_html.='<div style="height:20px;">
							<span id="price_'.$key.'">
							<label class="item-label2">'.$html.'</label>&nbsp;&nbsp;
							现价格<input type="text" name="price['.$key.']['.$xb.']" class="text input" value="">&nbsp;&nbsp;
							原价格<input type="text" name="oldprice['.$key.']['.$xb.']" class="text input" value="">&nbsp;&nbsp;
							库存<input type="text" name="stock['.$key.']['.$xb.']" class="text input" value="">
							</span>
						</div><br>';
		}
		header('content-type:application/json;charset=utf8');  
		$arr['status']=1;
		$arr['attr_str']=$attr_str;
		$arr['attr_html']=$attr_html;
		echo json_encode($arr);
		exit;
	}
