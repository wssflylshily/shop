...
function getuserattr($user_id,$field='login_user')
{
	//$user=M('User')->field($field)->where('id='.$user_id)->find();
	...
	return $name;//$user[$field];
}

function getproductattr($id,$field='title')
{
	$product=M('Product')->field($field)->where('id='.$id)->find();
...
//查询退款卡号
function getbankcard($bank_id){
    if ($bank_id<1)return '';
    $userbank =M('bank')->where('id ='.$bank_id)->find();
    return $userbank['card_num'];
}

//查询退款手机
function getbankphone($user_id){
    if ($user_id<1)return '';
    $userbank =M('bank')->where('user_id ='.$user_id)->order('id DESC')->find();
    return $userbank['mobile'];
}

//查询开户银行
function getbank($bank_id){
    if ($bank_id<1) return '';
    $bank =M('bank_type')->where('id ='.$bank_id)->find();
    return $bank['title'];
}
...
function getstorename($stroid){
	$store =M('store')->where('id ='.$stroid)->find();
	return $store['sname'];
}
...
function getGradelist(){
	$list = M('grade')->order('glv asc')->select();
	return $list;
}
...
function updateUserLevel($userid){
	...
	foreach($grade as $k=>$v){
		if ($user['integral']>=$v['glimit']){
			M('user')->where('id='.$userid)->save(array('lv'=>$v['id']));
		}
	}
}
...
function checkPhoneCode($phone,$code)
{
    ...
    $data_code['code'] = $code;
    $phone_code = M('phone_code')->where($data_code)->find();
    if (!$phone_code || time() > $phone_code['createtime']+60*5){
        ApiReturn("22001","短信验证码不正确或已超时");
    }
...
function checkPhoneImgCode($phone,$imgcode)
{
    ...
    if (!M('phone_code')->where($data_code)->find()){
        ApiReturn("22011","图片验证码不正确");
    }
}
