...
function getorderproducid($order_id=0)
{
	$order=M('Order')->where('id='.$order_id)->find();
	$map['order_id']=$order['id'];
	$detail=M('OrderDetail')->where($map)->order('id asc')->find();
...
function getProductgprice($product_id){
	$info =M('product')->where('id ='.$product_id)->find();
	return $info['gprice'];
}

function getProductmarketprice($product_id){
	$info =M('product')->where('id ='.$product_id)->find();
	return $info['market_price'];
}
...
function getBrokerage(){
	$info = M('brokerage')->getField('ratio_level1');
	if (!$info) $rate = 0;
	else $rate = $info/100;
	return $rate;
}
 ...
 * 获取二级分销佣金比例
 */
function getBrokerageTwo(){
	$info = M('brokerage')->getField('ratio_level2');
	if (!$info) $rate = 0;
	else $rate = $info/100;
	return $rate;
}
 ...
 * 获取三级分销佣金比例
 */
function getBrokerageThree(){
	$info = M('brokerage')->getField('ratio_level3');
	if (!$info) $rate = 0;
	else $rate = $info/100;
	return $rate;
}
 ...
 * 获取签到积分比例
 */
function getSignRatio(){
	$info = M('integrate')->find();
	if (!$info){
		$info['sign_ratio'] = 0;
	}
	return $info['sign_ratio'];
}
/**
 * 获取支付积分比例
*/
function getPayRatio(){
	$info = M('integrate')->find();
	if (!$info){
		$rate = 0;
	}else{
 ...
 * 根据用户id查找分销上级用户id
 */
function getParentUser($user_id){
	$parentid = M('user')->where('id='.$user_id)->getField('parent_user_id');
	return $parentid;
}